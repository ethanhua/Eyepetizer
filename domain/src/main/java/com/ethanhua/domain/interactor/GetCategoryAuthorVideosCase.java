...
public class GetCategoryAuthorVideosCase {
    private final VideoRepository mVideoRepository;
    private final ThreadTransformer mThreadTransformer;

    @Inject
    public GetCategoryAuthorVideosCase(VideoRepository videoRepository,
    ...
    public Single<ListData<ItemData<VideoListData>>> execute(long categoryId) {
        return mVideoRepository.listCategoryAuthor(categoryId).compose(mThreadTransformer);
    }
}
