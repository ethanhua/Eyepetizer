...
public class GetVideoListGroupByCategoryCase {

    private final VideoRepository mVideoRepository;
    private final ThreadTransformer mThreadTransformer;

    @Inject
    public GetVideoListGroupByCategoryCase(VideoRepository videoRepository,
                                           ThreadTransformer threadTransformer) {
        this.mVideoRepository = videoRepository;
        this.mThreadTransformer = threadTransformer;
    }

    public Single<ListData<ItemData<VideoListData>>> execute(Map map) {
        return mVideoRepository.listGroupByCategory(map).compose(mThreadTransformer);
    }
}
